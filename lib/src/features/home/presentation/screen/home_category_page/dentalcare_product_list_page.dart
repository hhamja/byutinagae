...
class DentalCareProductListPage extends ConsumerStatefulWidget {
  const DentalCareProductListPage({super.key});
...
class _CategoryProductListPageState
    extends ConsumerState<DentalCareProductListPage>
  ...
  void initState() {
    super.initState();
    _tabController =
        TabController(length: DentalCareCategory.values.length, vsync: this);
  ...
  Widget build(BuildContext context) {
    final provider =
        ref.watch(categoryProductsProvider(DentalCareCategory.toothpaste.name));
    // 4개로 나뉭진 카테고리에 따라 탭바랑 상위 title도 다르게 구성하기
    return DefaultLayout(
      leading: const CustomBackButton(),
      title: const Text('덴탈케어'),
        ...
        children: [
          ...
          Expanded(
            child: PageView(
              ...
              children: [
                CategoryProductListBody(query: provider),
              ],
            ),
          ),
        ],
